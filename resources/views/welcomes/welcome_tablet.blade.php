...
<?php $cont = 0; ?>
      ...
      <div class="card swiper-slide mb-2" style="width: 100%; height: 600px;">
        <img src="{{asset('img/'.$p->imagen_1)}}" class="card-img-top" alt="..." width="500" height="300" data-toggle="modal" data-target="#modal_accesorios_tablet_{{$cont}}">
        <div class="card-body">
        ...
        {!! Form::open(array('url'=>'agregar_carrito_cauchos', 'method'=>'POST', 'autocomplete'=>'off','id'=>'agregar_carrito_tablet_'.$cont, 'files'=>'true')) !!}
        ...
        <div class="input-group justify-content-center mb-2">
          <div class="input-group-prepend">
            <span class="input-group-text" id="incrementar_tablet_{{$cont}}" data="{{$cont}}">                  
              <i class="fas fa-plus" style="color: #4169E1;" data-container="body" data-toggle="popover" data-placement="top" data-content="1.Haz click e incrementa tu pedido" id="plus_tablet_{{$cont}}"></i>                
            </span>
          </div>
          <input type="number" class="text-center" name="cantidad" id="cantidad_tablet_{{$cont}}"  value="0" style="width: 60px;" data="{{$cont}}">
          <input type="hidden" id="producto_id_tablet_{{$cont}}" name="producto_id"  value="{{$p->id}}">
          <input type="hidden" id="inventario_tablet_{{$cont}}" name="inventario"  value="{{$p->inventario}}">
          <input type="hidden" id="precio_dolar_tablet_{{$cont}}" name="precio_dolar"  value="{{$p->precio_dolar}}">

          <div class="input-group-append">
            <span class="input-group-text" id="disminuir_tablet_{{$cont}}" data="{{$cont}}">                   
      ...
      <div class="card-body" style="text-align: center;">
        <button type="button" class="btn btn-danger" id="add_carrito_tablet_{{$cont}}" data-container="body" data-toggle="popover" data-placement="bottom" data-content="agrega un producto." data="{{$cont}}">
        Agregar al carrito
        <span class="badge badge-light">{{$cantidad_carrito}}</span>
          <span class="sr-only">unread messages</span>
      </button>
